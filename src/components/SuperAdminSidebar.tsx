...
import { 
  ...
  ChevronRight, LogOut, FileText, Key
} from "lucide-react";
...
export default function SuperAdminSidebar({ 
  ...
  return (
    <aside 
      ...
      className={`bg-[#0a0f24] border-r border-[#1e295d] text-slate-300 flex flex-col justify-between shrink-0 h-full overflow-hidden select-none transition-all duration-300 ease-in-out ${isCollapsed ? "w-[76px]" : "w-64"}`}
    >
      {/* Brand Section */}
      <div className={`p-4 border-b border-[#1e295d] bg-[#0c1330]/80 flex items-center ${isCollapsed ? "justify-center" : "justify-between"}`}>
        ...
        <button
          ...
          className={`p-1.5 rounded-lg bg-[#11193d] border border-[#23357a] text-indigo-400 hover:text-white transition shrink-0 ${isCollapsed ? "mt-1" : ""}`}
